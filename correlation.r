...
# corrplot
png("fig_correlations_numeriques.png", width = 800, height = 700, res = 100)
corrplot(cor_matrix,
         method   = "color",
         type     = "upper",
         addCoef.col = "black",
...
# Graphique age_estim en fonction du diamètre du tronc
png("fig_scatter_age_diametre.png", width = 750, height = 550, res = 100)
...
# Graphique age_estim en fonction de la hauteur totale
png("fig_scatter_age_hauteur.png", width = 750, height = 550, res = 100)
p <- ggplot(vars_num, aes(x = haut_tot, y = age_estim)) +
  geom_point(alpha = 0.25, color = "#4DAC26", size = 1.2) +
  geom_smooth(method = "lm", color = "#D73027", se = TRUE) +
...
analyze_age_estimations <- function(col1, col2, name1, name2, df, png_name) {
  ...
  # Create boxplot
  png(png_name, width = 750, height = 520, res = 100)
...
chi2_calc <- function(var1, var2, label1, label2, df, filename_prefix) {
  ...
  cat(sprintf("\n[%s ~ %s]\n", label1, label2))
  cat("Tableau croisé :\n"); print(tbl)
  chi2 <- chisq.test(tbl, simulate.p.value = TRUE, B = 2000)
  cat("Test du Chi2 :\n"); print(chi2)
...
chi2_calc("fk_stadedev", "fk_situation", "Stade de développement", "Situation",
         df, "fig_chi2_stadedev_situation")

# Stade de développement ~ Feuillage
chi2_calc("fk_stadedev", "feuillage", "Stade de développement", "Feuillage",
         df, "fig_chi2_stadedev_feuillage")

# Situation ~ Feuillage
chi2_calc("fk_situation", "feuillage", "Situation", "Feuillage",
         df, "fig_chi2_situation_feuillage")

# Stade de développement ~ Type de pied
chi2_calc("fk_stadedev", "fk_pied", "Stade de développement", "Type de pied",
         df, "fig_chi2_stadedev_pied")

# Situation ~ Remarquable
chi2_calc("fk_situation", "remarquable", "Situation", "Remarquable",
         df, "fig_chi2_situation_remarquable")

# Etat ~ Feuillage
chi2_calc("fk_arb_etat", "feuillage", "Etat", "Feuillage",
         df, "fig_chi2_etat_feuillage")
